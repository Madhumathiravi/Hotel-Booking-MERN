...
export default function Exclusive() {
  ...
  return (
    <div className="relative w-full font-poppins">
      ...
      <button className="exclusive-prev hidden lg:block absolute left-24 top-1/2 -translate-y-1/2 z-20 text-black">
        <FaArrowCircleLeft className="text-3xl" />
      </button>

      <button className="exclusive-next hidden lg:block absolute right-24 top-1/2 -translate-y-1/2 z-20 text-black">
        <FaArrowCircleRight className="text-3xl" />
      </button>
      ...
      <Swiper
        ...
        className="px-4 lg:px-[calc(50vw-510px)]"
      >
        {cards.map((card) => (
          <SwiperSlide key={card.id} className="transition-opacity duration-300 !w-auto flex justify-center">
            <ExclusiveCard card={card} />
          </SwiperSlide>
        ))}
      </Swiper>
      ...
      <div className="text-center mt-6">
        <button
          className="text-sm border rounded-3xl px-4 py-2 text-white bg-blue-500"
          onClick={() => navigate("/offers")}
        >
          View All
        </button>
